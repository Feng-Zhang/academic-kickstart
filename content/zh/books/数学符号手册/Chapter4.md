...
　　我们在第7页中谈过，假定一个集合A，注释|A|表示集合A的势。 对一个有限元素的集合来说，这是一个整数。但是对于无限的集合来说，我们有特别的表示方法。

　　符号$\aleph_0$用来表示整数集合的势：$\aleph_0 = |{\Bbb Z}|$。它是最小的超限数。符号${\aleph}$是希伯来语字母*aleph*，注释$\aleph_0$可以读作"aleph null" 或"aleph naught"。势为$\aleph_0$的集合也被称为*可数的*。
